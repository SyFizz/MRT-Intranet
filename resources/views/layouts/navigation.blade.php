...
<nav x-data="{ open: true }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                <img class="h-8 w-8 rounded-full object-cover" src="{{ Gravatar::get(Auth::user()->email ) }}"
                     alt="{{ Auth::user()->name }}"/>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('email-sender')" :active="request()->routeIs('email-sender')">
                {{ __('Messagerie') }}
            </x-responsive-nav-link>
        </div>
